...
# (4) Complete removal of some colonies due to replating 
   complete_lost= sample(x=SortOut$Cell_ID[SortOut$counts < 200], size=round(prob_firstpassage_death* length(SortOut$Cell_ID)), prob=SortOut$abund)
...
# 
# 
# lower.tri(cosine(as.matrix(cos_test)))
...
# have 5 replicates
# randomly choose c(10, 100, 1000, 10000) cells from experimental population without replacement. Compute average BC similarity

# num_test_reps = 5
# 
# rep_size = 10
# 
# fullset <- sample(x = experimental_population, size = rep_size*num_test_reps, replace = FALSE)
# 
# 
# table(fullset[1:rep_size])
# fullset[(rep_size+1):(2*rep_size)]
# fullset[(2*rep_size+1):(3*rep_size)]
# fullset[(3*rep_size+1):(4*rep_size)]
# fullset[(4*rep_size+1):(5*rep_size)]


#split_replicates <- split(fullset, ceiling(seq_along(fullset)/rep_size))  #split experimental population into even chunks


# all_replicate<- data.frame(matrix(ncol=length(fullset), nrow=0)) #create empty matrix based off number of Cell ID in Final Counts
# for (chunk in split_replicates)
# {
#   replicateN <- data.frame(table(chunk))                                                   # make replicate back into data frame
#   colnames(replicateN)<- c('Cell_ID', 'counts')                                                     # rename columns
...
#########################################################################################
# make sure order of outs matches columns of orginial empty df
