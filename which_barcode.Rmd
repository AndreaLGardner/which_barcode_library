...
mean_growth_rate = 0.0228   # mean growth rate of cell line to be used [1/hr]
...
############ SET INPUTS ####################################
```{r}
iterations = 9 # iterations per library size
...
ggplot(res, aes(x = library_size, y = final_total_evenness, group = library_size)) +
  geom_violin(fill='grey') + geom_jitter(width=0.01) + theme_few() +
  ylim(0.85, 1) +
  scale_x_log10() +
...
```{r}
sims <- res %>% select(library_size, experimental_BC, experimental_SI, experimental_evenness) %>% 
  rename('aliquot_BC' = 'experimental_BC',
         'shannon'    = 'experimental_SI',
         'evenness'   = 'experimental_evenness')


write_csv(sims, 'which_barcode_outs.csv')
```
