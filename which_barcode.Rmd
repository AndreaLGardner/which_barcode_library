...
grow_cells <- function(df, t, K)
  {
    df_out <- df %>% select(-abund) %>%  
             mutate(counts = floor(solveN(g = gr, N0 = counts, tf = t, k = K)),     # solve logistic growth for N at expansion time for each Cell_ID        
...
t_expand = solvet(g = SortMax_gr, N0 = sum(SortMatrix$counts), Nfinal = 0.80*k_96well, k = k_96well)   # solve logistic growth equation for expansion time


### EXPAND CELLS
# (4) Simulate growth of each subclone to t_expand and store new cell counts and abundance in data frame
SortGrow <- grow_cells(df = SortMatrix, t = t_expand, K = k_96well)
